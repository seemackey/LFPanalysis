function [fixationstats] = ClusterFix01(eyedat,samprate, duration_threshold_for_saccades, duration_threshold_for_fixations,fltord,lowpasfrq)
...
% duration_threshold_for_saccades     = 8;
% duration_threshold_for_fixations    = 25;
% fltord = 60;
% lowpasfrq = 100;
...
for cndlop = 1:length(eyedat)
    if size(eyedat{cndlop},2) > 500/samprate/1000
        ...
        T = kmeans(points,numclusters(end),'replicate',5,'MaxIter',1000);
        ...
        for ii = 1:size(fixationtimes,2);
            ...
            for numclusts = 1:5
                ...
                T = kmeans(POINTS(1:5:end,:),numclusts,'replicate',5,'MaxIter',1000);
                [silh] = InterVSIntraDist(POINTS(1:5:end,:),T);
                sil(numclusts) = mean(silh);
            end
            sil(sil > 0.9*max(sil)) = 1;
            numclusters = find(sil == max(sil));  %it's dangerous to have too many clusters
            T = kmeans(POINTS,ceil(median(numclusters)),'replicate',5,'MaxIter',1000);
        ...
        tooshort = find(diff(saccadetimes,1) < duration_threshold_for_saccades); %10 ms duration threshold for saccades
        notbehav = [];
        for ii = 1:length(tooshort);
        ...
        [fixationtimes] = BehavioralIndex(fixationindexes);
        fixationtimes(:,(diff(fixationtimes,1) < duration_threshold_for_fixations))= []; %25 ms duration threshold for fixations
        fixationindexes = [];
        for ii = 1:size(fixationtimes,2);
