function [eegcb, eegbb, trig01, trigtype, eegmb, pupepdb,pupepd] = EphysAndPupilExtractFxn(path,trigch)
...
timeframe_baseline              = [-500 0];
epoch_tframe            = [-500 1000];  
newadrate =500;
filtere = [0.5 300];%LFP
...
%% sort trigs based on input trigger "trigch"
...
x2 = round(epoch_tframe(2)*(newadrate/1000));
    ...
    for i=1:length(trig01)
         ...
         pupepd(i,:)=cai.cnt(4,trig01(i)+x1:trig01(i)+x2); % same for pupil
