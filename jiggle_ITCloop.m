...
%% set directory
% tic
mydir = 'F:\dyneyep\jitter\contproc\wavelet\gt038_01'; %gets directory
...
for loopct = 1:length(myfiles)
   ...
   mididx = time(1,:)==mid;
    ...
    for chanct = 1:1:numchans
        ...
        itc.pvals{loopct,chanct} = squeeze(wraw.avg.itcpc{1}(1,chanct,frqbnd,mididx)); 
        
        power.all{loopct,chanct} = squeeze(wraw.avg.poc{1}(1,chanct,frqbnd,:)); % power across time
